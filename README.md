...
- 分层：
  - `widget` —— 悬浮球本体：绘制、拖拽、贴边竖条、右键菜单、悬停轮询
...
| 右键菜单 | 设置 / 系统监视器 / 退出；系统监视器命令可在设置里自定义（留空自动检测，单实例启动） |
...
3. **数据中转站**：鼠标在悬浮球上停留约 300ms（或把文件拖到球上）即弹出面板：
   ...
   - **四种视图**：底栏左侧切换 图标网格 / 列表 / 详细 / 预览；「详细」视图显示
     每条的类型、大小、路径与文本摘要。
   - **类型筛选**：标题下沿 tab「全部 / 文档 / 图片」只筛"看什么"，不动数据与顺序。
   - **预览**：鼠标悬停条目弹出预览（图片原图 / 文本内容 / 文件图标 / 视频静音播放），
     预览下沿有「类型 · 尺寸 · 大小 · 完整路径」详情条；移开即收起。
...
| `[system_monitor]` | `cmd` | 右键「系统监视器」自定义命令，留空自动检测 |
